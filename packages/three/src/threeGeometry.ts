...
export class ThreeGeometry extends ThreeVisualObject implements IVisualGeometry {
    ...
    private initEdges(data: EdgeMeshData) {
        if (data.lineType === "dash") {
            // Dashed edges (e.g. construction geometry) are comparatively rare
            // and can carry their own color (unlike the shared default material
            // below), so build a one-off geometry+material via the same factory
            // the temporary preview/highlight overlays already use.
            this._edges = ThreeGeometryFactory.createEdgeGeometry(data);
        } else {
